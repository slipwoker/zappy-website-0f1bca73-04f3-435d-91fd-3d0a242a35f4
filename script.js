document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (value.trim() !== '') {
        if ((type === 'email' || field.getAttribute('data-type') === 'email') && !validators.email(value)) {
          showError(field, getErrorMessage(field, 'email'));
          return false;
        }

        if ((type === 'tel' || field.getAttribute('data-type') === 'tel') && !validators.tel(value)) {
      ...
      if ('IntersectionObserver' in window) {
        ...
        animatableElements.forEach(function (el, index) {
          if (!el.hasAttribute('data-delay')) {
            const siblings = el.parentNode ? el.parentNode.querySelectorAll(OBSERVE_SELECTORS.join(', ')) : [];
            let siblingIndex = 0;
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Carmi & Cohen משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '0f1bca73-04f3-435d-91fd-3d0a242a35f4',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '0f1bca73-04f3-435d-91fd-3d0a242a35f4';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '0f1bca73-04f3-435d-91fd-3d0a242a35f4';
